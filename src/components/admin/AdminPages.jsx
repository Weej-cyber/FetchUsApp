...
function ClientsAndWalkersSection() {
  ...
  async function handleAdd(e) {
    e.preventDefault()
    setSaving(true)
    await supabase.auth.signInWithOtp({ email: form.email, options: { shouldCreateUser: true, data: { name: form.name, phone: form.phone, role: addingRole } } })
    await supabase.from('users').upsert({ email: form.email, name: form.name, phone: form.phone || null, role: addingRole }, { onConflict: 'email' })
    setSaving(false)
