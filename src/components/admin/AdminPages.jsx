// AdminPages.jsx — Phase 1 Complete Rewrite
// Single-page scrolling portal: Stats → Manage → Schedule → Clients
...
function StatCard({ label, value, color = '#5B4B8A' }) {
  return (
    <div style={{
      ...
      boxShadow: '0 2px 12px rgba(45,52,54,0.08)', borderTop: `4px solid ${color}`,
    }}>
...
function EmptyState({ message }) {
  return (
    <div style={{ textAlign: 'center', padding: '24px 0', color: '#b2bec3', fontSize: '0.88rem' }}>{message}</div>
  )
}

const labelStyle = {
  display: 'block', fontSize: '0.78rem', fontWeight: 700,
  color: '#636e72', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.03em',
}
const inputStyle = {
  width: '100%', border: '1.5px solid #E0E0E0', borderRadius: 8,
  padding: '8px 10px', fontSize: '0.88rem', fontFamily: 'inherit',
  outline: 'none', boxSizing: 'border-box', background: 'white',
}
const saveBtnStyle = {
  background: '#5B4B8A', color: 'white', border: 'none',
  borderRadius: 8, padding: '8px 18px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer',
}
const cancelBtnStyle = {
  background: 'white', color: '#636e72', border: '1.5px solid #E0E0E0',
  borderRadius: 8, padding: '8px 14px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer',
}
...
function WalkRequestCard({ req, walkers, onDecline, onAssign }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: '0 2px 8px rgba(45,52,54,0.07)', marginBottom: 12,
      borderLeft: '4px solid #5B4B8A',
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: '0.95rem', color: '#2D3436' }}>
            {req.dogs?.name ?? 'Unknown dog'} — {req.service_type}
          </div>
          <div style={{ fontSize: '0.83rem', color: '#636e72', marginTop: 2 }}>
            {formatDate(req.preferred_date)} at {req.preferred_time}
          </div>
          <div style={{ fontSize: '0.8rem', color: '#b2bec3', marginTop: 2 }}>
            {req.clients?.users?.name ?? 'Unknown client'} · {timeAgo(req.created_at)}
          </div>
        </div>
        <span style={{
      ...
      {req.notes && (
        <div style={{ fontSize: '0.82rem', color: '#636e72', background: '#FAF8F3', borderRadius: 8, padding: '8px 10px', marginBottom: 10 }}>
          "{req.notes}"
        </div>
      )}

      {req.assigned_walker_id && (
        <div style={{ fontSize: '0.8rem', color: '#2D9B8A', fontWeight: 600, marginBottom: 8 }}>
          Assigned to: {walkers.find(w => w.id === req.assigned_walker_id)?.name ?? 'Unknown'}
        </div>
      )}
      ...
      {req.status === 'pending' && (
        <div style={{ display: 'flex', gap: 8, marginTop: 4, flexWrap: 'wrap' }}>
          <button
            ...
            style={{ background: 'white', border: '1.5px solid #FCA5A5', color: '#991B1B', borderRadius: 8, padding: '6px 14px', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer' }}
          >
            Decline
          </button>
          <button
            onClick={() => setShowAssign(!showAssign)}
            style={{ background: '#5B4B8A', color: 'white', border: 'none', borderRadius: 8, padding: '6px 14px', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer' }}
          >
            Assign Walker
          </button>
        </div>
      ...
      {showAssign && req.status === 'pending' && (
        <div style={{ marginTop: 10, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <select
            ...
            style={{ borderRadius: 8, border: '1.5px solid #DDD6FE', padding: '6px 10px', fontSize: '0.85rem', flex: 1, minWidth: 140, background: 'white' }}
          >
          ...
          <button
            ...
            disabled={!selectedWalker || assigning}
            style={{ background: '#2D9B8A', color: 'white', border: 'none', borderRadius: 8, padding: '6px 14px', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', opacity: !selectedWalker ? 0.5 : 1 }}
          >
...
function BroadcastPanel() {
  ...
  async function sendBroadcast() {
    ...
    const { data: clients } = await supabase
      .from('users').select('id, phone').eq('role', 'client').not('phone', 'is', null)
    if (clients?.length) {
      await supabase.from('notifications').insert(
        clients.map(c => ({ user_id: c.id, type: 'broadcast', message: message.trim(), phone: c.phone, status: 'pending' }))
      )
    }
    setSending(false)
    setSent(true)
  ...
  return (
    <div style={{ background: 'white', borderRadius: 12, padding: 18, boxShadow: '0 2px 8px rgba(45,52,54,0.07)', marginTop: 16 }}>
      ...
      <textarea
        ...
        rows={3}
        style={{ width: '100%', border: '1.5px solid #E0E0E0', borderRadius: 8, padding: '10px 12px', fontSize: '0.88rem', fontFamily: 'inherit', resize: 'vertical', outline: 'none', boxSizing: 'border-box' }}
      />
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
        ...
        <button
          ...
          style={{ background: sent ? '#2D9B8A' : '#5B4B8A', color: 'white', border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', opacity: !message.trim() ? 0.5 : 1 }}
        >
...
function ScheduleSection({ walkers }) {
  ...
  async function loadWalks() {
    ...
    const { data } = await supabase
      .from('walk_requests')
      .select('*, dogs(name), clients(users(name)), assigned_walker:users!assigned_walker_id(name)')
  ...
  return (
    <div>
      <SectionHeader
        title="Schedule"
        action={
          <button onClick={() => setShowAddForm(!showAddForm)} style={saveBtnStyle}>+ Add Walk</button>
        }
      />
...
function ClientsSection() {
  ...
  async function loadClients() {
    setLoading(true)
    const { data } = await supabase
      .from('users').select('id, name, email, phone, created_at').eq('role', 'client').order('name')
  ...
  async function handleAddClient(e) {
    ...
    await supabase.auth.signInWithOtp({
      email: form.email,
      options: { shouldCreateUser: true, data: { name: form.name, phone: form.phone, role: 'client' } }
    })
    await supabase.from('users').upsert({ email: form.email, name: form.name, phone: form.phone || null, role: 'client' }, { onConflict: 'email' })
    setSaving(false)
  ...
  return (
    <div>
      <SectionHeader
        title="Clients"
        action={<button onClick={() => setShowAddForm(!showAddForm)} style={saveBtnStyle}>+ Add Client</button>}
      />

      {showAddForm && (
        <form onSubmit={handleAddClient} style={{ background: 'white', borderRadius: 12, padding: 18, boxShadow: '0 2px 8px rgba(45,52,54,0.07)', marginBottom: 16, borderLeft: '4px solid #D4A843' }}>
          <div style={{ fontWeight: 700, marginBottom: 14, color: '#2D3436' }}>
            {magicLinkSent ? '✓ Magic link sent!' : 'Add Client'}
          </div>
          {!magicLinkSent && (
            <>
              <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                <button type="button" onClick={() => setShowAddForm(false)} style={cancelBtnStyle}>Cancel</button>
                <button type="submit" disabled={saving} style={saveBtnStyle}>{saving ? 'Sending...' : 'Send Magic Link'}</button>
              </div>
            </>
          )}
        </form>
      ...
      {loading ? <EmptyState message="Loading clients..." />
        : filtered.length === 0 ? <EmptyState message={search ? 'No clients match your search.' : 'No clients yet.'} />
        : filtered.map(c => (
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <div style={{ fontSize: '0.8rem', fontWeight: 700, color: '#636e72', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 10 }}>
          ...
          {pendingRequests.length > 0 && (
            <span style={{ background: '#FEF9C3', color: '#92400E', borderRadius: 12, padding: '1px 7px', marginLeft: 6, fontSize: '0.75rem' }}>
          ...
          : (
            <>
              {pendingRequests.map(r => (
                <WalkRequestCard key={r.id} req={r} walkers={walkers} onDecline={handleDecline} onAssign={handleAssign} />
              ))}
