...
function ClientsSection() {
  ...
  async function loadClients() {
    setLoading(true)
    const { data } = await supabase.from('users').select('id, name, email, phone, created_at').eq('role', 'client').order('name')
  ...
  async function handleAddClient(e) {
    e.preventDefault()
    setSaving(true)
    await supabase.auth.signInWithOtp({ email: form.email, options: { shouldCreateUser: true, data: { name: form.name, phone: form.phone, role: 'client' } } })
  ...
  return (
    <div>
      <SectionHeader title="Clients" action={<button onClick={() => setShowAddForm(!showAddForm)} style={saveBtnStyle}>+ Add Client</button>} />
      {showAddForm && (
        <form onSubmit={handleAddClient} style={{ background: 'white', borderRadius: 12, padding: 18, boxShadow: '0 2px 8px rgba(45,52,54,0.07)', marginBottom: 16, borderLeft: '4px solid #D4A843' }}>
          <div style={{ fontWeight: 700, marginBottom: 14, color: '#2D3436' }}>{magicLinkSent ? '✓ Magic link sent!' : 'Add Client'}</div>
          {!magicLinkSent && (
            <>
              <div style={{ marginBottom: 10 }}>
      ...
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search clients..." style={{ ...inputStyle, marginBottom: 12, width: '100%', boxSizing: 'border-box' }} />
...
export default function AdminPortal() {
  ...
  async function loadRequests() {
    ...
    const { data } = await supabase.from('walk_requests').select('*, dogs(name), clients(users(name))').order('created_at', { ascending: false }).limit(20)
