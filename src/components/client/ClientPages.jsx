  ...
  return (
    <div style={{ background: colors.cream, minHeight: '100vh', maxWidth: '430px', margin: '0 auto', fontFamily: 'Nunito' }}>
      ...
      <div style={{ padding: '24px', paddingBottom: '100px' }}>
        {services.map(s => (
          <div key={s.name} style={{ background: 'white', borderRadius: '16px', padding: '16px 20px', marginBottom: '12px', boxShadow: '0 2px 12px rgba(45,52,54,0.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontWeight: 700, color: colors.charcoal }}>{s.name}</span>
            <span style={{ fontWeight: 800, color: colors.indigo }}>{s.price}</span>
          </div>
        ))}
        <p style={{ fontSize: '0.8rem', color: '#636e72', textAlign: 'center', marginTop: '8px' }}>+$5 for evenings, weekends & holidays</p>
      </div>
      <BottomNav />
    </div>
  )
  ...
  return (
    <div style={{ background: colors.cream, minHeight: '100vh', maxWidth: '430px', margin: '0 auto', fontFamily: 'Nunito' }}>
      ...
      <div style={{ padding: '24px', paddingBottom: '100px', textAlign: 'center', paddingTop: '60px', color: '#636e72' }}>
        ...
        <button style={{ padding: '14px 32px', borderRadius: '12px', background: `linear-gradient(135deg, ${colors.indigo}, #7B6BA8)`, color: 'white', border: 'none', fontFamily: 'Nunito', fontWeight: 700, fontSize: '1rem', cursor: 'pointer' }}>+ Add a Dog</button>
      </div>
      <BottomNav />
    </div>
  )
  ...
  return (
    <div style={{ background: colors.cream, minHeight: '100vh', maxWidth: '430px', margin: '0 auto', fontFamily: 'Nunito' }}>
      ...
      <div style={{ padding: '24px', paddingBottom: '100px' }}>
        ...
        <button onClick={signOut} style={{ width: '100%', padding: '14px', borderRadius: '12px', background: 'white', color: colors.indigo, border: `2px solid ${colors.indigo}`, fontFamily: 'Nunito', fontWeight: 700, fontSize: '1rem', cursor: 'pointer' }}>Sign Out</button>
      </div>
      <BottomNav />
    </div>
  )
