...
const DEMO_PROFILE = {
  ...
  address: '123 Main Street', access_instructions: 'Gate code: 1234. Please use side entrance.',
}
...
export default function ClientPortal() {
  ...
  const [profile, setProfile] = useState({ name: '', email: '', phone: '', address: '', access_instructions: '' })
  ...
  async function loadAll() {
    ...
    try {
      ...
      const { data: clientData } = await supabase.from('clients').select('id, address, access_instructions').eq('user_id', user.id).single()

      if (clientData) {
        setClientId(clientData.id)
        setProfile({ name: userData?.name || '', email: userData?.email || user.email || '', phone: userData?.phone || '', address: clientData.address || '', access_instructions: clientData.access_instructions || '' })
        ...
        setProfile({ name: userData?.name || '', email: userData?.email || user.email || '', phone: userData?.phone || '', address: '', access_instructions: '' })
      }
    } catch (err) {
      console.error(err)
      setDemoMode(true)
