// Book.jsx — Phase 1 Step 1.7
// Writes to walk_requests table
...
export default function Book() {
  ...
  if (loading) {
    return <div style={{ maxWidth: 600, margin: '0 auto', padding: '40px 16px', fontFamily: 'Nunito, sans-serif', textAlign: 'center', color: '#b2bec3' }}>Loading...</div>
  }

  return (
    <div style={{ maxWidth: 600, margin: '0 auto', padding: '20px 16px 100px', fontFamily: 'Nunito, sans-serif' }}>
      <div style={{ marginBottom: 24 }}>
        <button onClick={() => navigate(-1)} style={{ background: 'none', border: 'none', color: '#5B4B8A', fontSize: '0.9rem', fontWeight: 700, cursor: 'pointer', padding: 0, marginBottom: 12 }}>
          ← Back
        </button>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ background: 'white', borderRadius: 12, padding: 18, boxShadow: '0 2px 8px rgba(45,52,54,0.07)', marginBottom: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {SERVICE_TYPES.map(type => (
              <button key={type} type="button" onClick={() => setForm({ ...form, service_type: type })} style={{
                border: form.service_type === type ? '2px solid #5B4B8A' : '2px solid #E0E0E0',
                background: form.service_type === type ? '#F3F0FF' : 'white',
                color: form.service_type === type ? '#5B4B8A' : '#636e72',
                borderRadius: 8, padding: '10px 6px', fontSize: '0.85rem',
                fontWeight: form.service_type === type ? 700 : 600, cursor: 'pointer', fontFamily: 'inherit',
              }}>
                {type}
              </button>
            ))}
        ...
        <div style={{ background: 'white', borderRadius: 12, padding: 18, boxShadow: '0 2px 8px rgba(45,52,54,0.07)', marginBottom: 12 }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {TIME_SLOTS.map(slot => (
                <button key={slot} type="button" onClick={() => setForm({ ...form, preferred_time: slot })} style={{
                  border: form.preferred_time === slot ? '2px solid #2D9B8A' : '2px solid #E0E0E0',
                  background: form.preferred_time === slot ? '#ECFDF5' : 'white',
                  color: form.preferred_time === slot ? '#065F46' : '#636e72',
                  borderRadius: 8, padding: '10px', fontSize: '0.88rem',
                  fontWeight: form.preferred_time === slot ? 700 : 600, cursor: 'pointer', fontFamily: 'inherit',
                }}>
                  {slot}
                </button>
              ))}
        ...
        {error && (
          <div style={{ background: '#FEE2E2', color: '#991B1B', borderRadius: 8, padding: '10px 14px', fontSize: '0.85rem', marginBottom: 12 }}>{error}</div>
        )}

        <button type="submit" disabled={submitting || !form.preferred_date || !form.preferred_time} style={{
          width: '100%', background: '#5B4B8A', color: 'white', border: 'none',
          borderRadius: 12, padding: '14px', fontSize: '1rem', fontWeight: 700,
          cursor: 'pointer', opacity: (!form.preferred_date || !form.preferred_time) ? 0.5 : 1, fontFamily: 'inherit',
        }}>
          {submitting ? 'Sending Request...' : 'Send Request 🐾'}
        </button>

        <p style={{ textAlign: 'center', fontSize: '0.8rem', color: '#b2bec3', marginTop: 10 }}>
